...
exports.renderScanning = renderScanning;
...
function renderViolation(v) {
    ...
    if (v.policyDetail) {
        ...
        if (pd.riskDescription) {
            rows.push(`<div class="detail-row"><strong>Risk</strong><div>${linkify(pd.riskDescription)}</div></div>`);
        }
        if (pd.effortDescription) {
            rows.push(`<div class="detail-row"><strong>Effort</strong><div>${linkify(pd.effortDescription)}</div></div>`);
        }
        if (pd.downtimeDescription) {
            rows.push(`<div class="detail-row"><strong>Downtime</strong><div>${linkify(pd.downtimeDescription)}</div></div>`);
        }
...
function renderTagViolation(v) {
    ...
    if (v.invalidTags && v.invalidTags.length > 0) {
        tagList += v.invalidTags.map(t => `<div class="tag-list"><strong>${esc(t.key)}:</strong> <code>${esc(t.value)}</code>${t.suggestion ? ` (suggestion: <code>${esc(t.suggestion)}</code>)` : ""}${t.validValues && t.validValues.length > 0 ? `<div class="tag-list">Valid values: ${t.validValues.map(v => `<code>${esc(v)}</code>`).join(", ")}</div>` : ""}</div>`).join("");
    }
    return `
    <div class="violation">
...
function linkify(s) {
    return esc(s)
        .replace(/&lt;a href=&quot;(.*?)&quot;(.*?)&gt;(.*?)&lt;\/a&gt;/g, (_, url, _attrs, text) => `<a href="${url.replace(/&amp;/g, "&")}">${text}</a>`)
        .replace(/`([^`]+)`/g, '<code>$1</code>');
}
//# sourceMappingURL=resourceHtml.js.map